...
const projectSummaryPath = path.resolve(docsDir, '../project/PROJECT_SUMMARY.md');
const logEntriesPath = path.resolve(docsDir, 'log/sessions.md');

/**
 * Archives session notes older than the most recent 5 sessions
 * from project/PROJECT_SUMMARY.md to log/entries.md
 */
async function archiveSessionNotes() {
  try {
    ...
    console.log(`Found ${sessions.length} sessions in project/PROJECT_SUMMARY.md`);
    ...
    console.log(`- ${recentSessions.length} recent sessions kept in project/PROJECT_SUMMARY.md`);
